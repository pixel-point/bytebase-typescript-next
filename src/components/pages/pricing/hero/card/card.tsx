...
const Card = ({
  ...
  return (
    <article
      className="bg-white h-full border border-gray-70 shadow-pricing flex flex-col items-center border-t-8 pt-3.5 md:pt-2.5 pb-10 px-6 lg:px-5 md:px-4.5 md:pb-9"
      style={{ borderTopColor: planColor }}
    >
      <div className="flex flex-col items-center min-h-[548px] lg:min-h-[525px] w-full md:min-h-[462px]">
        <h2
        ...
        <p className="mt-2 lg:mt-3.5 text-gray-30 text-16 leading-normal lg:leading-snug text-center lg:max-w-[180px] md:mt-1 md:text-14">
          {description}
        </p>
        ...
        {additionalDescription && (
          <p
            className="mt-3 with-medium-text text-14 leading-tight tracking-tight text-gray-40 md:leading-snug text-center md:mt-4 md:max-w-[150px]"
            dangerouslySetInnerHTML={{ __html: additionalDescription }}
          />
        )}
      </div>
      <footer className="border-t border-gray-70 pt-6 w-full md:pt-5">
        <ul className="flex flex-col gap-y-4">
          {planConditions.map((condition, index) => (
            <li className="flex items-center lg:items-start gap-x-2" key={index}>
              ...
              <span className="text-15 lg:leading-normal leading-none text-gray-15 md:text-14 md:leading-snug">
                {condition}
              </span>
            </li>
          ))}
